...
template_1 = """
    <threat xsi:type="spartamodel:ThreatType" name="%s">
    %s
    </threat>
"""
...
def threat(type, threats):
    ...
    for name, desc, attackers, victims in threats:
        for atk, vic in zip(attackers, victims):
            res += threat_pattern(name, desc, atk, vic)
...
def generate_xml():
    ...
    root = ET.Element("spartamodel:ThreatTypeCatalog")
    root.set('xmi:version', '2.0')
    root.set('xmlns:xmi', 'http://www.omg.org/XMI')
    root.set('xmlns:xsi', 'http://www.w3.org/2001/XMLSchema-instance')
    root.set('xmlns:spartamodel', 'http://distrinet.cs.kuleuven.be/spartamodel')
    ...
    for file_name in os.listdir('catalog'):
        if file_name.endswith('.json'):
            with open(os.path.join('catalog', file_name), 'r') as f:
                ...
                aa = []
                threat = ET.SubElement(root, "threat")
                threat.set('xsi:type', 'spartamodel:ThreatType')
                threat.set('name', name)
                for victim in victims:
                    if 'Container' in victim or 'VirtualMachine' in victim:
                        continue
                    victim = victim.replace('su:comp_', '')
                    for attacker in attackers:
                        if 'Container' in attacker  or 'VirtualMachine' in victim:
                            continue
                        attacker = attacker.replace('su:comp_', '')
                        victim = change_name(victim)
                        attacker = change_name(attacker)
                        aa.append((attacker, victim))
                aa = list(set(aa))
                for a in aa:
                    patterns = ET.SubElement(threat, "patterns")
                    patterns.set('key', a[0] + 'To' + a[1])
                    patterns.set('value', 'true')



    tree = ET.ElementTree(root)
    ET.indent(tree)
    tree.write("IACThreatTypeCataLog.sparta", xml_declaration=True, encoding='utf-8', method="xml")
